...
func (w *Watcher) reloadClients(rescanAuth bool, affectedOAuthProviders []string, forceAuthRefresh bool) {
	...
	totalNewClients := authFileCount + geminiAPIKeyCount + vertexCompatAPIKeyCount + claudeAPIKeyCount + codexAPIKeyCount + xaiAPIKeyCount + commandCodeAPIKeyCount + openAICompatCount
...
func BuildAPIKeyClients(cfg *config.Config) (gemini, vertex, claude, codex, xai, commandcode, openAICompat, liteLLM int) {
	...
	if len(cfg.InteractionsKey) > 0 {
		gemini += len(cfg.InteractionsKey)
	}
	if len(cfg.VertexCompatAPIKey) > 0 {
		vertex += len(cfg.VertexCompatAPIKey)
	}
